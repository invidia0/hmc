...
@partial(jax.jit, static_argnames=['dVdq', 'negative_log_prob'])
def integrate_block(key, q0, p0, dVdq, negative_log_prob, step_size, block_len, direction, collection):
    """
...
def trajectory_sampling(traj_len, block_len, collection, initial_params, step_size, key):
    """
    ...
    (_, q, p, rep_weight), traj_chain = jax.lax.scan(body_fn, (key, rep_q, rep_p, rep_weight), block_len, length=3+1)
...
def hmc_sample(n_samples, block_len, collection, initial_params, traj_len, step_size, key):

    def one_sample(carry, _):
        key, q = carry
        key, subkey = jax.random.split(key)
        p0 = jax.random.normal(subkey, shape=q.shape)  # resample momentum each step

        q_new = trajectory_sampling(
            traj_len=traj_len,
            block_len=block_len,
            collection=collection,
            initial_params=(q, p0),
        ...
        return (key, q_new), q_new

    (_, _), samples = jax.lax.scan(one_sample, (key, initial_params), None, length=n_samples)
    return samples
...
if __name__ == "__main__":
    ...
    init_batched = jnp.tile(initial, (n_chains, 1))
    rng_init = jax.random.PRNGKey(42)
    init_batched = init_batched + 0.1 * jax.random.normal(rng_init, init_batched.shape)
    step_size = 0.1
    integration_time = int(3)

    master_key = jax.random.PRNGKey(0)
    chain_keys = jax.random.split(master_key, n_chains)
    ...
    batched_hmc_sample = jax.jit(batched_hmc)

    block_len = 2 ** jnp.arange(integration_time + 1, dtype=jnp.int32)
    dim = initial.shape[0]
    collection = (
        jnp.zeros((2**integration_time, dim)),  # q
        jnp.zeros((2**integration_time, dim)),  # p
        jnp.zeros((2**integration_time,))       # energies or log probs
    )
    start_time = time.time()
    samples = batched_hmc_sample(
        n_samples,
        block_len,
        collection,
        init_batched,
